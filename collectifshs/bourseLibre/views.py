...
def profil_contact(request, user_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            sujet = "[collectifshs] "+ request.user.username + "(" + request.user.email+ ") vous a écrit: "+ form.cleaned_data['sujet']
            message_txt = ""
...
class ListeProduit(ListView):
    ...
    def get_qs(self):
        ...
        if "collectifshs" in params and self.request.user.is_permacat:
            if params['collectifshs'] == "True":
...
def agora_permacat(request, ):
    ...
    if form.is_valid():
        ...
        message.save()
        group, created = Group.objects.get_or_create(name='collectifshs')
...
def contacter_newsletter(request):
    ...
    if request.method == 'POST':
        form = ContactForm(request.POST or None, )
        if form.is_valid():
            sujet = "[collectifshs] Newsletter - " +  form.cleaned_data['sujet']
            ...
            try:
                ...
                sujet = "[collectifshs admin] Erreur lors de l'envoi du mail"
                message_txt = message + '\n'.join(emails)
