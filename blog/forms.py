...
from marcheLibre.settings import SUMMERNOTE_CONFIG as summernote_config
...
class ArticleForm(forms.ModelForm):
   # contenu = TinyMCE(attrs={'cols': 80, 'rows': 20})
    estPublic = forms.ChoiceField(choices=((1, "Article public"), (0, "Article CollectifsHL")), label='', required=True, )
    ...
    def save(self, userProfile, sendMail=True):
        ...
        if not userProfile.is_collectifshl:
...
class ProjetForm(forms.ModelForm):
    ...
    #contenu = TinyMCE(attrs={'cols': 80, 'rows': 20})
    estPublic = forms.ChoiceField(choices=((1, "Projet public"), (0, "Projet CollectifsHL")), label='', required=True)
